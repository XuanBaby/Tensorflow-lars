...
#        x -= tf.constant(MEAN_RGB, dtype=tf.float32)
#        x /= tf.constant(STDDEV_RGB, dtype=tf.float32)
#        x -= tf.constant(MEAN_RGB,shape=[1,3,1,1], dtype=tf.float32)
#        x /= tf.constant(STDDEV_RGB,shape=[1,3,1,1], dtype=tf.float32)
        return x
...
##########################################################################################################################################################
    ...
    def batch_norm_relu(self, inputs, relu=True, init_zero=False,data_format='channels_first'):
    ...
    def dropblock(self, net, keep_prob, dropblock_size,data_format='channels_first'):

        if not self.is_training or keep_prob is None:
            return net
    ...
    def residual_block(self, inputs, filters, is_training, strides,use_projection=False, data_format='channels_first'):
        ...
        if use_projection:
            ...
            shortcut = conv2d_fixed_padding(
                ...
                data_format=data_format)
            shortcut = batch_norm_relu(shortcut, relu=False,
                                    data_format=data_format)

        inputs = conv2d_fixed_padding(
            inputs=inputs, filters=filters, kernel_size=3, strides=strides,
            data_format=data_format)
        inputs = batch_norm_relu(inputs, data_format=data_format)

        inputs = conv2d_fixed_padding(
            inputs=inputs, filters=filters, kernel_size=3, strides=1,
            data_format=data_format)
        inputs = batch_norm_relu(inputs, relu=False, init_zero=True,
                                data_format=data_format)

        return tf.nn.relu(inputs + shortcut)


    def bottleneck_block(self, inputs, filters, strides, use_projection=False, data_format='channels_first'):
        ...
        if use_projection:
            ...
            shortcut = self.conv2d_fixed_padding(
                ...
                data_format=data_format)
            shortcut = self.batch_norm_relu(shortcut, relu=False,
                                    data_format=data_format)
        ...
        inputs = self.conv2d_fixed_padding(
            ...
            data_format=data_format)
        inputs = self.batch_norm_relu(inputs, relu=False, init_zero=False, data_format=data_format)

        return tf.nn.relu(inputs + shortcut)

    def block_group(self, inputs, filters, block_fn, blocks, strides,  name,data_format='channels_first'):
    ...
    def __init__(self, preprocessor, loss_func, nstep_per_epoch=None):
        ...
        with tf.device('/cpu:0'):
            self.global_step = tf.get_variable(
                ...
                trainable=False)
#        if FLAGS.warmup:
#     	    warmup_steps = int(nstep_per_epoch * 5)
#            warmup_lr = (
#                FLAGS.learning_rate * tf.cast(self.global_step,tf.float32) / tf.cast(
...
#                staircase=True)
        #self.learning_rate = tf.where(self.global_step < warmup_steps, warmup_lr,learning_rate_decay)
        if FLAGS.warmup:
            # Learning rate schedule for LARS polynomial schedule
            if FLAGS.batch_size * hvd.size() < 8192:
                plr = 5.0
                w_epochs = 5
            elif FLAGS.batch_size * hvd.size() < 16384:
                plr = 10.0 
                #25.0
                w_epochs = 5
            elif FLAGS.batch_size * hvd.size() < 32768:
                plr = 25.0 
		#34.0
                w_epochs = 5 
		#12
		...
		#41.0
                w_epochs = 14 
		#16
            warmup_steps = int(nstep_per_epoch * w_epochs)
            warmup_lr = (
                plr * tf.cast(self.global_step,tf.float32) / tf.cast(
                warmup_steps,tf.float32))
            train_step = FLAGS.num_epochs*nstep_per_epoch
            min_step = tf.constant(1,dtype=tf.int64)
            decay_steps = tf.maximum(min_step, tf.subtract(self.global_step, warmup_steps))
            poly_rate = tf.train.polynomial_decay(
                plr,
                decay_steps,
                train_step - warmup_steps + 1,
                power=2.0)
            self.learning_rate = tf.where(self.global_step <= warmup_steps, warmup_lr, poly_rate)
    ...
    def training_step(self, batch_size):
        ...
        # Apply the gradients to optimize he loss function
		#opt = hvd.DistributedOptimizer(opt)
        with tf.device('/gpu:0'):
            opt = tf.train.MomentumOptimizer(self.learning_rate, FLAGS.momentum,
                                             use_nesterov=False)		
            #opt = lo.LARSOptimizer(self.learning_rate,FLAGS.momentum,FLAGS.weight_decay,eeta=0.001,epsilon=0.0,skip_list=['batch_normalization'],use_nesterov=False)
            # Horovod: (optional) compression algorithm.
            compression = hvd.Compression.fp16 if FLAGS.fp16_allreduce else hvd.Compression.none

            opt = hvd.DistributedOptimizer(opt, device_dense='/gpu:0', device_sparse='/gpu:0', compression=compression)
...
#                true_vals = tf.scalar_mul(LARC_eta, tf.div(v_norms, g_norms))
                true_vals = tf.scalar_mul(LARC_eta, tf.div(v_norms, (g_norms + FLAGS.weight_decay * v_norms)))
...
#######################
def tpu_resnet(net, input_layer):
    resnet_depth = 50
    ...
    model_params = {
        18: {'block': net.residual_block, 'layers': [2, 2, 2, 2]},
        34: {'block': net.residual_block, 'layers': [3, 4, 6, 3]},
        50: {'block': net.bottleneck_block, 'layers': [3, 4, 6, 3]},
        101: {'block': net.bottleneck_block, 'layers': [3, 4, 23, 3]},
        152: {'block': net.bottleneck_block, 'layers': [3, 8, 36, 3]},
        200: {'block': net.bottleneck_block, 'layers': [3, 24, 36, 3]}}
    ...
    inputs = net.input_layer(input_layer)
    print("1232_row:",inputs)
    #tf.cast(inputs,tf.float32)
    inputs = net.conv2d_fixed_padding(
        inputs=inputs, filters=64, kernel_size=7, strides=2,
        data_format=data_format)
    inputs = tf.identity(inputs, 'initial_conv')
    inputs = net.batch_norm_relu(inputs, data_format=data_format)
    ...
    inputs = net.block_group(
        inputs=inputs, filters=64, block_fn=block_fn, blocks=layers[0],
        strides=1, name='block_group1',
        data_format=data_format)
    inputs = net.block_group(
        inputs=inputs, filters=128, block_fn=block_fn, blocks=layers[1],
        strides=2, name='block_group2',
        data_format=data_format)
    inputs = net.block_group(
        inputs=inputs, filters=256, block_fn=block_fn, blocks=layers[2],
        strides=2, name='block_group3',
        data_format=data_format)
    inputs = net.block_group(
        inputs=inputs, filters=512, block_fn=block_fn, blocks=layers[3],
        strides=2, name='block_group4',
        data_format=data_format)
    ...
    print("1270_1271_row:",inputs)
    print("block_fn:",id(block_fn))
    print("net.bottleneck_block:",id(net.bottleneck_block))
    print("block_fn is net.bottleneck_block:",block_fn is net.bottleneck_block)
    inputs = tf.reshape(
        inputs, [-1, 2048 if block_fn == net.bottleneck_block else 512])
    print("1273_row:",inputs)
    return inputs
#######################
...
#                         choices=['inception3', 'resnet50'])
	...
	#FLAGS.save_model_steps
    cmdline.add_argument('--save_model_steps',default=1000,type=int,
    ...
    cmdline.add_argument('--fp16-allreduce',action='store_true',default=False,
                        help='use fp16 compression dtring allreduce')
    cmdline.add_argument('--warmup',action='store_true',default=False,
                        help="Builds scaled learning rate function with 5 epoch warm up.")
...
######################################################################################
    elif model_name.startswith('tpu_resnet'):
        height, width = 224, 224
        model_func = lambda net, images: tpu_resnet(net, images)
        #if FLAGS.learning_rate == None:
        #    FLAGS.learning_rate = 1. * (batch_size * hvd.size() / 1024.0) if nlayer > 18 else 0.5
#######################################################################################
...
#            params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
#                                       scope=var_scope.name)
#            l2_loss = tf.add_n([tf.nn.l2_loss(v) for v in params if 'beta' not in v.name and "gamma" not in v.name])
#            if l2_loss.dtype != tf.float32:
#                l2_loss = tf.cast(l2_loss, tf.float32)	
#            loss += FLAGS.weight_decay * l2_loss
        return loss, logits
    ...
    #compute time of build graph
    time_BuildGraph_start = time.time()
    if FLAGS.eval:
        ...
        total_loss, learning_rate, train_ops = trainer.training_step(
            batch_size)
    time_BuildGraph_end = time.time()
    print('"time of BuildGraph in" %d "is" %f' % (hvd.rank(),(time_BuildGraph_end - time_BuildGraph_start)))

    print_r0("Creating session")
    #print('"Creating session : " %d' % hvd.rank()) 
    ...
    #HC
    print("**********************")
    print(str(int(hvd.local_rank()/hvd.size())))
    print("**********************")
    config.gpu_options.visible_device_list = str(int(hvd.local_rank()/hvd.size()))
    ...
    if hvd.rank() == 0 and len(FLAGS.log_dir):
        ...
        saver = tf.train.Saver(max_to_keep=0,keep_checkpoint_every_n_hours=3)
        last_save_time = time.time()

    if not FLAGS.eval:
        #compute time of initialize variable
        print('"Initializing variables : " %d ' % hvd.rank())
        trainer.init(sess)
   ...
   #         options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE)
    #        run_metadata = tf.RunMetadata()
            ...
            if (hvd.rank() == 0 and summary_ops is not None and
                ...
                with open('timeline_rank_0.json','w') as f:
...
#            (step + 1) % FLAGS.save_model_steps == 0 or step+1 == nstep):
